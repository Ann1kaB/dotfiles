...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, iswine;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
applyrules(Client *c)
{
	...
	if (strstr(class, ".exe") || strstr(class, "steam_app_"))
		c->iswine = 1;
...
setfocus(Client *c)
{
	...
	/* restore focus of window so that wine/proton applicationscan resume properly */
	if (c->iswine)
		setclientstate(c, NormalState);
}
